...
function Model({modelUrl,animationName,scale=1,position=[0,0,0],rotation=[0,0,0],playbackSpeed=1}){
  ...
  useEffect(()=>{
    ...
    const exact=names.find(n=>n.toLowerCase()===String(animationName||'').toLowerCase());
    const fuzzy=names.find(n=>n.toLowerCase().includes(String(animationName||'').toLowerCase()));
    ...
    return()=>{action.fadeOut(.15);action.stop();};
...
export default function Exercise3DViewer({
  ...
  if(!modelUrl)return <div className="exercise3d-empty" style={{height:viewerHeight}}>Δεν υπάρχει 3D μοντέλο.</div>;
